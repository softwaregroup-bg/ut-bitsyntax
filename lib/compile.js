...
function get_string(segment) {
  ...
  if (segment.z) {
    ...
    if (segment.binhex) {
      let varName = var_name(segment.size);
      $line(`var orgSize = ${varName}`);
      $line(`${varName} = (${varName} % 8) ? parseInt(${varName}) + (8 - ${varName} % 8) : ${varName};`);
    }
    $line("bits = %s;", bits_expr(segment));
    $line("offset += bits;");
    $line("if (offset > binsize) { return false; }");
    ...
    if (segment.hexbin) {
      ...
      $line(" result = bin.toString('hex', byteoffset, byteoffset + bits / 8).slice(-orgSize); }");
    } else if (segment.bcd) {
