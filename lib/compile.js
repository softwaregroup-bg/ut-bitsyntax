...
function bits_expr(segment) {
  if (typeof segment.size === 'string') {
    var name = var_name(segment.size);
    if (segment.type === 'string' && (segment.bcd || segment.binhex) && segment.unit === 4) {
      return $('(%s + (%s % 2)) * %d; if (!Number.isInteger(bits))  {return false; }', name, name, segment.unit);
    }
    return $('%s * %d; if (!Number.isInteger(bits))  {return false; }', name, segment.unit);
  }
  else {
...
function get_string(segment) {
  ...
  if (segment.z) {
    ...
    if (segment.hexbin) {
      $line(" result = (new Buffer(result, 'hex')); }");
    } else if (segment.binhex) {
      $line(" result = bin.toString('hex', byteoffset, byteoffset + bits / 8); }");
      if (typeof segment.size === 'string' && segment.unit === 4) {
        $line(" var defSize = %s", var_name(segment.size));
        $line(" result = result.slice(-defSize); ");
      }
    } else if (segment.bcd) {
      $line(" result = bin.toString('hex', byteoffset, byteoffset + bits / 8); }");
      if (typeof segment.size === 'string' && segment.unit === 4) {
