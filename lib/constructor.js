    ...
    case 'string':
      if(segment.value != undefined){
        ...
        if (segment.hexbin) {
          ...
          var valLen = bindings[segment.name].length;
          valLen += valLen % 2;
          val = paddString(bindings[segment.name], '0', valLen, 'left');
...
function size_of_segment(segment, bindings) {
  ...
  if (segment.type === 'string' || segment.type === 'binary') {
    ...
    if (segment.type === 'string') {
      return segment.value != undefined ? Buffer.byteLength(segment.value, 'utf8') : segment.size ? (segment.size * segment.unit) / 8 : Buffer.byteLength(bindings[segment.name] || '', 'utf8');
    }
  }
