    ...
    case 'string':
      if(segment.value != undefined){
        ...
        if (segment.left != undefined || segment.right != undefined) {
          ...
          val = paddString(val, padChar, size_of_segment(segment, bindings) * 2, padDirection);
        }
        if (segment.hexbin) {
          val = (new Buffer(val)).toString('hex').toUpperCase();
...
function size_of_segment(segment, bindings) {
  ...
  if (typeof segment.size === 'string') {
    var size = bindings[segment.size];
    if ((segment.bcd || segment.binhex) && segment.unit === 4) {
      size += size % 2;
    }
    return size * segment.unit / 8;
  }
