    ...
    case 'string':
      if(segment.value != undefined){
        ...
        if (segment.left != undefined || segment.right != undefined) {
          ...
          val = paddString(val, padChar, size_of_segment(segment, bindings), padDirection);
        }
        if (segment.hexbin) {
          val = (new Buffer(bindings[segment.name])).toString('hex').toUpperCase();
        } else if (segment.binhex) {
          val = new Buffer(bindings[segment.name].toString(), 'hex');
        } else if (segment.bcd) {
          val = bindings[segment.name];
          if (!/^\d*$/.test(val)) throw new Error("Non decimal value for field '" + segment.name + "'!");
          val = new Buffer(bindings[segment.name], 'hex');
        } else if (segment.xbcd) {
          val = bindings[segment.name];
          val = paddString(val, '0', 2 * size_of_segment(segment, bindings), 'left');
          if (!/^\d*$/.test(val)) throw new Error("Non decimal value for field '" + segment.name + "'!");
...
function size_of_segment(segment, bindings) {
  ...
  if(typeof segment.size === 'string' && segment.xbcd) {
    var bcdLen = (bindings[segment.size] * segment.unit) / 8;
    return (bcdLen % 2 ? bcdLen + 1 : bcdLen) / 2;
  }
  ...
  if (segment.type === 'string' || segment.type === 'binary') {
    if (segment.z) {
      var s = bindings[segment.name];
      if (s == null) {
        return 0;
      }
      if ((typeof s !== 'string') && !Buffer.isBuffer(s)) {
        throw new Error("Field " + segment.name + " must be String or Buffer");
      }
      var realSize = s.length;
...
function writeBcd(segment, buf, offset, bindings) {
  ...
  var resBuf = intToBcd(value, size);
